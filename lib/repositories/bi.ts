/**
 * BI repository — unified read queries for business intelligence data.
 *
 ...
 *
 * NOTE: getBIAlertas and getBenchmarkSetor are NOT unified here because
 * alertas has mutation-adjacent logic (dispensar/reativar) and benchmark
 * has unique cross-empresa aggregation. They stay in actions.ts / multi-queries.ts.
 *
 * Actually, per the task spec, ALL BI functions are reads. We unify them all.
 */
...
export async function getBIFilterOptionsRepo(
  client: SupabaseClient,
  empresaIds: string[],
): Promise<{ data: BIFilterOptions | null; error: string | null }> {
  try {
    // Build OR filter for categorias: empresa_id is null OR in empresaIds
    const empresaFilter = empresaIds.map(id => `empresa_id.eq.${id}`).join(',');
    ...
    const [camRes, motRes, catRes] = await Promise.all([
      ...
      client
        ...
        .or(`empresa_id.is.null,${empresaFilter}`)
        .eq('ativa', true)
        .order('ordem'),
    ]);
...
export async function getBIKpisRepo(
  client: SupabaseClient,
  empresaIds: string[],
  filtros: BIFiltros,
): Promise<{ data: BIKpis | null; error: string | null }> {
  try {
    let viagemQuery = client
      .from('viagem')
...
export async function getBIMargemMotoristasRepo(
  client: SupabaseClient,
  empresaIds: string[],
  filtros: BIFiltros,
...
export async function getBICategoriasBreakdownRepo(
  client: SupabaseClient,
  empresaIds: string[],
  filtros: BIFiltros,
...
export async function getBIRankingCaminhoesRepo(
  client: SupabaseClient,
  empresaIds: string[],
  filtros: BIFiltros,
...
export async function getBIRankingMotoristasRepo(
  client: SupabaseClient,
  empresaIds: string[],
  filtros: BIFiltros,
  ...
  try {
    ...
    const byDriver = new Map<string, {
      motoristaId: string; nome: string;
      totalGasto: number; qtdLancamentos: number;
    }>();
...
export async function getBITendenciaMensalRepo(
  client: SupabaseClient,
  empresaIds: string[],
  filtros: BIFiltros,
...
export async function getBIEficienciaCombustivelRepo(
  client: SupabaseClient,
  empresaIds: string[],
  filtros: BIFiltros,
  ...
  try {
    ...
    // Group trips by truck and calculate km (trip + gap)
    ...
    // Aggregate fuel data by truck
    const fuelByTruck = new Map<string, {
      placa: string; modelo: string;
      totalLitros: number; totalGastoCentavos: number; totalAbastecimentos: number;
    }>();
...
export async function getBIEficienciaMotoristasRepo(
  client: SupabaseClient,
  empresaIds: string[],
  filtros: BIFiltros,
  ...
  try {
    ...
    // Group trips by driver and calculate km
    ...
    // Aggregate fuel data by driver
    const fuelByDriver = new Map<string, {
      nome: string; totalLitros: number;
...
export async function getBIManutencoesRepo(
  client: SupabaseClient,
  empresaIds: string[],
  filtros: BIFiltros,
...
export async function getBIAlertasRepo(
  client: SupabaseClient,
  empresaIds: string[],
  filtros: BIFiltros,
  ...
  try {
    ...
    // 1. Consumo excessivo de combustivel
    {
      let viagemQuery = client
        .from('viagem')
    ...
    const { data: dispensados } = await client
